...
subset_selection <- function(y,X,M) {
  ...
  # Initialise
  # 
  b <- matrix(rep(0,p),p,1)
  r <- as.vector(y)
  e <- as.vector(rep(0.01,n))
  
  for(m in 2:M) {
    ## select x_j with highest correlation with r
    j <- which.max(cor(x=r,y=X))
    xj <- X[,j]

    a <- suppressWarnings(e*sign(t(xj)%*%r))
    b <- suppressWarnings(cbind(b,b[j,]+a))
    r <- suppressWarnings(r-a%*%xj)
    
  }
  
  return(b)
}
subset_selection(y,X,100)
```
...
matplot(ss.b,pch=20, cex=.5)
plot(lm.b$coefficients[-1], pch=20, cex=.5)

```

Curry `dgf` and `subset_selection`.
